...
#早期版本的MySQL数据库的information_schema数据库中没有名为COLUMN_STATISTICS的数据表。
...
**6.启动服务找不到pid**
```no-highlight
mysql5.7.18之后的版本就不需要再路径/etc下面配置my.cnf文件就可以启动，
但是如果需要修改端口，开启MySQL的慢查询或者时主从复制的功能还是需要在/etc下面配置my.cnf文件的（扩展有配置参考）。

我自己的解决方案是使用默认的pid，log等相关的路径，即不配置pid，log等
#
# include all files from the config directory
# 删除这段引入  因为/etc/my.cnf.d里的mysql-server.cnf有配置pid，log等路径
#
...
否则如果配置的路径中没有文件、文件夹则又没有写入权限则会报错
```
...
```hgignore
查找pid文件位置
使用命令ps -ef|grep mysql即可,--pid-file=这个参数后面的值就是该应用对应的pid文件路径

[root@admin conf]# ps -ef|grep mysql
 mysql     2937     1  0 08:53 ?        00:00:00 /bin/sh /usr/bin/mysqld_safe --basedir=/usr
 mysql     3167  2937  0 08:53 ?        00:00:15 /usr/sbin/mysqld -- basedir=/usr --datadir=/var/lib/mysql --plugin-dir=/usr/lib64/mysql/plugin --log-error=/var/log/mysqld.log --pid-file=/var/run/mysqld/mysqld.pid --socket=/var/lib/mysql/mysql.sock --port=3306
 root      5084  3905  0 18:51 pts/2    00:00:00 grep --color=auto mysql
```

```hgignore
my.cnf配置参考，如果需要那些可自行拷走一部分用，但是不要全部拷上去，否则可能会导致mysql启动不了
